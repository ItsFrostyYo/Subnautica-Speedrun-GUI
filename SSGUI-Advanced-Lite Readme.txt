──────────────────────────────────────────────────────────────
      Subnautica Speedrun GUI — Advanced Version (Release V1.5)
                     By ItsFrosti


SSGUI (Advanced Version) provides a streamlined way to manage multiple
...
Lite Version removes Save File Installer and Speedrun Timer ,
This make it half the size allowing for a liter and possibly faster experience.

Advanced uses Livesplit Component with the Game to Connect a Pipeline for Information
...
2. LiveSplit Data Connection
   - Notification when the Pipeline Successfully Connects or Disconnects.
   - 1. Launch Subnautica and GUI 2. Launch LiveSplit with "Subnautica Data" Layout Component.

*How to Setup Correctly for Use

Find Livesplit Install
  - Find "LiveSplit.SubnauticaData.dll" inside the SSGUI Folder
    - Move it into your LiveSplit Components Folder. 
          "\LiveSplit_X.X.XX\'->Components<-'"

Open Livesplit and Rick Click for the Menu,
 Press "Edit Layout…"
  Press the + Button
   Press on "Control"
    Finally Select "Subnautica Data"

*Save this Component with the Layout and you are Finished.

3. Use the GUI to configure all features and set hotkeys.

4. The app communicates internally with the GUI using a text document
...
**Setup Process (6 Steps)**
1. Hover over the **Quit** button → press **TAB**
2. Hover over the **1st Yes** button → press **TAB**
3. Hover over the **2nd Yes** button → press **TAB**
4. Hover over the **Play** button → press **TAB**
5. Hover over **Start a New Game** → press **TAB**
6. Hover over desired **Game Mode** → press **TAB**
...
**Connection with Livesplit**
- When connected with LiveSplit it Reads the Coordinates of the Game.
  - Understands if you are on the Main Menu by Checking Exact Game Coordinates.
  - Most Consistent and Efficient compared to Pixel Check and Screen Coordinates.
...
- If **"SSGUI-Advanced.exe"** fails to launch, install **AutoHotkey v1**.
...
- For Pipeline to work you must Launch LiveSplit after GUI is Open.
